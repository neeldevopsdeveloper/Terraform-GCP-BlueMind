locals {
  secret_ids = {
    db_password              = var.db_secret_id
    stripe_secret_key        = var.stripe_secret_key
    stripe_publishable_key   = var.stripe_publishable_key
    stripe_monthly_price_id  = var.stripe_monthly_price_id
    stripe_annual_price_id   = var.stripe_annual_price_id
    canada_tax_rate_ids_prod = var.canada_tax_rate_ids_prod
  }

  secret_names = {
    for key, value in local.secret_ids :
    key => element(
      reverse(
        split(
          "/",
          split("/versions/", value)[0]
        )
      ),
      0
    )
    ...
    spec {
      ...
      containers {
        ...
        env {
          ...
          value_from {
            secret_key_ref {
              name = local.secret_names["stripe_secret_key"]
              key  = "latest"
            }
          }
        }
        ...
        env {
          ...
          value_from {
            secret_key_ref {
              name = local.secret_names["stripe_publishable_key"]
              key  = "latest"
            }
          }
        }
        ...
        env {
          ...
          value_from {
            secret_key_ref {
              name = local.secret_names["stripe_monthly_price_id"]
              key  = "latest"
            }
          }
        }
        ...
        env {
          ...
          value_from {
            secret_key_ref {
              name = local.secret_names["stripe_annual_price_id"]
              key  = "latest"
            }
          }
        }
        ...
        env {
          ...
          value_from {
            secret_key_ref {
              name = local.secret_names["canada_tax_rate_ids_prod"]
              key  = "latest"
            }
          }
        }
